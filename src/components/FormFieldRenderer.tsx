import { TextField, MenuItem, Checkbox, FormControlLabel, RadioGroup, Radio, FormGroup } from '@mui/material'
...
export default function FormFieldRenderer({ field, value, onChange, error }: Props) {
  ...
  if (field.type === 'text' || field.type === 'number' || field.type === 'date') {
    ...
    if ((field.options ?? []).length > 0) {
      return (
        <FormGroup>
          {(field.options ?? []).map(o => (
            <FormControlLabel
              key={o}
              control={<Checkbox checked={Array.isArray(value) ? value.includes(o) : false} onChange={(e) => {
                const arr = Array.isArray(value) ? [...value] : []
                if (e.target.checked) arr.push(o); else {
                  const idx = arr.indexOf(o); if (idx>=0) arr.splice(idx,1)
                }
      ...
      return <FormControlLabel control={<Checkbox checked={!!value} onChange={(e) => onChange(field.id, e.target.checked)} />} label={field.label} />
    }
  }
